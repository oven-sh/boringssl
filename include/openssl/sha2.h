...
// add an extra define here since 'sha512_final_impl' asserts that the
// intermediary work buffer has a length % 8. Once the hashing is done it's
// free to be truncated to the define above losslessly.
#define SHA512_224_DIGEST_BUFFER_LENGTH 32
...
// digest to |out|, which must have at least |SHA512_224_DIGEST_BUFFER_LENGTH|
// bytes of space. It returns one on success and zero on programmer error.
// Upon success |out| can be truncated to |SHA512_224_DIGEST_LENGTH| bytes.
OPENSSL_EXPORT int SHA512_224_Final(
    uint8_t out[SHA512_224_DIGEST_BUFFER_LENGTH], SHA512_CTX *sha);

// SHA512_224 writes the digest of |len| bytes from |data| to |out| and returns
// |out|. There must be at least |SHA512_224_DIGEST_BUFFER_LENGTH| bytes of
// space in |out|.
OPENSSL_EXPORT uint8_t *SHA512_224(
