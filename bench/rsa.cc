...
void BM_SpeedRSAVerify(benchmark::State &state, bssl::Span<const uint8_t> der) {
  bssl::UniquePtr<RSA> key(RSA_private_key_from_bytes(der.data(), der.size()));
  if (!key || RSA_size(key.get()) > kMaxSignature) {
    state.SkipWithError("Failed to parse key.");
    return;
  ...
  for (auto _ : state) {
    if (!RSA_verify(NID_sha256, fake_sha256_hash, sizeof(fake_sha256_hash), sig,
                    sig_len, key.get())) {
      state.SkipWithError("RSA_verify failed.");
      return;
    }
  }
}

void BM_SpeedRSAImportKeyAndVerify(benchmark::State &state,
...
void BM_SpeedRSAParsePrivateKey(benchmark::State &state,
                                bssl::Span<const uint8_t> der) {
  for (auto _ : state) {
...
#define RSA_TEST_SUITE(size, key)                             \
  ...
  BENCHMARK_CAPTURE(BM_SpeedRSAVerify, size, key)             \
      ->Apply(bssl::bench::SetThreads);                       \
  BENCHMARK_CAPTURE(BM_SpeedRSAImportKeyAndVerify, size, key) \
      ->Apply(bssl::bench::SetThreads);

BSSL_BENCH_LAZY_REGISTER() {
  RSA_TEST_SUITE(2048, kDERRSAPrivate2048)
  RSA_TEST_SUITE(3072, kDERRSAPrivate3072)
  RSA_TEST_SUITE(4096, kDERRSAPrivate4096)
}
